...
class DistinctiveDishModel {
    // الحصول على جميع الأطباق المميزة النشطة
         static async getAllDishesByActivity() {
            ...
            for (const row of rows) {
                const isCurrentlyActive = row.is_active === 1 && (!row.end_date || new Date(row.end_date) >= new Date());
    ...
    static async addDish(dishData) {
        try {
            const [result] = await connection.promise().query('INSERT INTO distinctive_dishes SET ?', dishData);
            ...
            if (error.code === 'ER_NO_REFERENCED_ROW_2') {
                userMessage = 'Cannot add dish: dish_id does not exist in dishes table.';
    ...
    static async updateDish(id, updates) {
        await connection.promise().query('UPDATE distinctive_dishes SET ? WHERE id = ?', [updates, id]);
    }

    // حذف طبق مميز
    static async deleteDish(id) {
        await connection.promise().query('DELETE FROM distinctive_dishes WHERE id = ?', [id]);
    }
}
